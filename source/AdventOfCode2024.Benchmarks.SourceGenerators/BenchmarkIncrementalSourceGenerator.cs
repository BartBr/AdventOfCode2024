...
public class BenchmarkIncrementalSourceGenerator : IIncrementalGenerator
{
	...
	{
		var incrementalValueProvider = context.CompilationProvider.SelectMany(static (compilation, cancellationToken) =>
		{
		...
		var lastPuzzleIncrementalValueProvider = incrementalValueProvider
			.Collect()
			.Select((x, ct) => x.LastOrDefault()?.Key);

		context.RegisterSourceOutput(lastPuzzleIncrementalValueProvider, (productionContext, day) =>
		{
			productionContext.AddSource(
				"LastPuzzleBenchmark.generated.cs",
				SourceText.From($@"
// <auto-generated/>
...
", Encoding.UTF8));
	...
	private void GenerateBenchmarkClasses(SourceProductionContext productionContext, IGrouping<string, BenchmarkPuzzleInfo> benchmarkPuzzleContext)
	{
		...
		sourceWriter.WriteLine();
		sourceWriter.WriteLine($"public class {benchmarkPuzzleContext.Key}Benchmark");
		sourceWriter.WriteLine('{');
		sourceWriter.Indentation++;

		foreach (var benchmarkPuzzleInfo in benchmarkPuzzleContext)
		{
		...
		sourceWriter.WriteLine($"public System.Collections.Generic.IEnumerable<AdventOfCode2024.Common.Input> ValuesForInput => AdventOfCode2024.Common.Helpers.GetAllInputs(\"{benchmarkPuzzleContext.Key}\");");
		...
		productionContext.AddSource($"{benchmarkPuzzleContext.Key}Benchmark.generated.cs", sourceWriter.ToSourceText());
		return;

		static string GenerateUserSpecificBenchmarkMethods(string name, string firstPartReturnType, string secondPartReturnType)
		{
			return $@"
